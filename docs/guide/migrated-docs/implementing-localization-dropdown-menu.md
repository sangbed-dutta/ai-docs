...
2. Give a name to the Variable, say *menudata*, and set the **isList** option to true.
...
:::note
Here, we have parameterized the *mylabel* values as KEY\_1, KEY\_2, KEY\_3, KEY\_4, KEY\_5 so that the label values can be displayed as per the locale selection at runtime.
:::
...
4. Drag and drop a Dropdown Menu widget on the canvas ([more on Dropdown Menu](#))
   ...
   - Under the Actions section set:
     - the Action Label to the following value: *App.appLocale\[mylabel],* (you have to use the Use Expression option from the Bind Dialog)
