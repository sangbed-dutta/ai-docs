...
function getPageContext() {
  if (typeof document === 'undefined') {
    return { pageTitle: '', pageSlug: '', pageCategory: '', pageHeadings: [], pageSummary: '' };
  }
  ...
  if (article) {
    const parts = [];
    article.querySelectorAll('p').forEach((p) => parts.push(p.textContent?.trim() || ''));
...
export default function AskAIDialog({ apiUrl, onClose, initialQuery = '' }) {
  ...
  if (chat.activeMessageId === '__streaming__') {
    ...
    const activeMsg = assistantMessages.find((m) => m.id === chat.activeMessageId);
    if (activeMsg) {
      activeCards = activeMsg.sourceCards || [];
      activeIndex = assistantMessages.findIndex((m) => m.id === chat.activeMessageId) + 1;
      const msgIdx = chat.messages.findIndex((m) => m.id === chat.activeMessageId);
  ...
  const handlePrevMessage = useCallback(() => {
    const idx = assistantMessages.findIndex((m) => m.id === chat.activeMessageId);
        ...
        {!hasMessages ? (
          ...
          <div className={styles.chatLayout}>
              ...
              <div className={styles.chatInputArea}>
                <div className={styles.chatInputWrapper}>
                  ...
                  <button
                    ...
                    onClick={() => { chat.clearHistory(); setInput(''); }}
                    title="Clear history"
                  >
                ...
                <p className={styles.chatInputHint}>Enter to send · Shift+Enter for new line</p>
              </div>
            </div>
            ...
            <SourceCards
              ...
              totalMessages={chat.isStreaming ? totalAssistantMessages + 1 : totalAssistantMessages}
